    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.image is not None:
                ...
                if len(bbs) >0:
                    bbs = utils.non_max_suppresion(bbs, confidence_threshold=0.50, diff_class_thresh=0.75)
                    image = utils.draw_bb_on_image(self.array_image, bbs, utils.CLASS_DICT)
                    ros_img = self.bridge.cv2_to_imgmsg(cv2.cvtColor(image, cv2.COLOR_BGR2RGB),  encoding="passthrough")
