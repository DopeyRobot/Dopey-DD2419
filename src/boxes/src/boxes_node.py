...
from tf2.transformation import quaternion_from_euler
...
class BoxNode:
    def __init__(self) -> None:
        ...
        self.distance_threshold = 0.2 # distance between box and obstacle to still be considered a box
    ...
    def get_obstacles_from_laserscan(self):
        obstacle_map_poses = []
        try:
            transform_camera2map = self.buffer.lookup_transform(self.reference_frame, "camera_depth_frame", rospy.Time(0)) # Is the order of source and target frame correct?
        ...
        for dist, angle in list(zip(self.laserscan.ranges, angles))[::10]:
            ...
            obstacle_map_poses.append(distancePoseStamped) #tf2_geometry_msgs.do_transform_pose(distancePoseStamped, transform_camera2map) #continuous coordinates
    ...
    def find_closest_obstacle(self, box, obstacles:List[PoseStamped])-> List[str]:
        ...
        for i in range(len(obstacles)):
            dist = abs(box - obstacles[i])
            if  dist < self.distance_threshold: # possibly need to compute the substraction for the poses (i.e. the distance between the two poses)
                if dist < prev_dist:
                    prev_dist = dist
                    idx_closest_pt = i
        if idx_closest_pt != 0:
    ...
    def measure_box(self, obstacle_poses, idx):
        ...
        for obstacle in obstacle_poses[idx:]:
            ...
            if abs(current_pt.pose.position.y - obstacle.pose.position.y) > self.delta:
                right_end = obstacle
                break
        ...
        for obstacle in obstacle_poses[idx::-1]:
            #TODO: CHECK which direction the front for the laserscan !!!!!!!!!!!!!!!!!!!!
            if abs(obstacle.pose.position.y - current_pt.pose.position.y) > self.delta:
                left_end = obstacle
                break
        ...
        box_center.pose.position.x = (left_end.pose.position.x + right_end.pose.position.x)/2 # this is in map frame and should be in camera depth frame
        ...
        if abs(box_corner.pose.position.y - left_end.pose.position.y) and abs(box_corner.pose.position.y - right_end.pose.position.y) > .2:
