...
class RRTPlanner:
    ...
    def sample_random(self)->Tuple[int]:
        if random.random() > self.goal_sample_prob:
            return (np.random.randint(0, map_data.shape[0]), np.random.randint(0, map_data.shape[1]))
        else:
    ...
    def find_nearest(self, x, y) -> RRTNode:
        ...
        for node in self.RRT:
            if np.linalg.norm(np.array([x, y]) - np.array([node.x, node.y])) < np.linalg.norm(np.array([x,y]) - np.array([nearest_node.x, nearest_node.y])):
                nearest_node = node
        return nearest_node
    ...
    def generate_RRT(self):
        for i in range(self.num_iterations):
            ...
            if new_node is not None:
                ...
                if np.linalg.norm(np.array([new_node.x, new_node.y]) - np.array(self.goal)) <= self.step_size:
                    print("Found goal!")
    ...
    def plot_RRT_tree(self):
        ...
        for i, node in enumerate(self.RRT):   
            ...
            self.ax.plot([node.x, parent.x], [node.y, parent.y], color='blue', alpha=0.3)
