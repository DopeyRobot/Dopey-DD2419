#!/usr/bin/env python3
import rospy
from geometry_msgs.msg import PolygonStamped, Point32, PoseStamped, Pose, Twist
...
class Workspace():
    def __init__(self):
        ...
        # self.dutycycle_zero = DutyCycles()
    ...
    def plannertwist_callback(self, twistmsg):
        self.plannertwist = twistmsg # use entire twist message created by planner
        ...
        while not rospy.is_shutdown():
            ...
            if self.robot_position is not None and self.navgoal_position is not None:
                ...
                if self.robot_inside:
                    ...
                    # robot is outside of the polygon workspace
                    # such that the desired is zero
                    twist_zero = Twist()
                    ...
                    encoder_zero.delta_encoder_right = 0
                    self.publisher_encoders.publish(encoder_zero)

                    rospy.loginfo("Stop robot since outside of workspace.")
            self.rate.sleep()
