...
class aruco_dectection:
    def __init__(self):
        self.sub_goal = rospy.Subscriber('/arucotray/markers', MarkerArray, self.aruco_callback)
        self.br = tf2_ros.TransformBroadcaster()
        self.buffer = tf2_ros.Buffer()
        self.listener = tf2_ros.TransformListener(self.buffer)
        self.aruco_markers_frame = "usb_cam_frame"
    ...
    def aruco_callback(self, msg):
        ...
        for detected_marker in msg.markers:
            if detected_marker.id in range(10,26):
...
if __name__ == '__main__':
    rospy.init_node('display_marker_tray')
    aruco_dectection()
    rospy.spin()